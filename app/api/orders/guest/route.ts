...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const order = await client.query('INSERT INTO ticketug.order (id,public_id,order_number,purchaser_name,purchaser_email,guest_access_token_hash,total_minor_units,idempotency_key) VALUES ($1,$2,$3,$4,$5,$6,$7,$8) RETURNING public_id,order_number,status,total_minor_units,currency', [orderId, `ord_${randomBytes(12).toString('hex')}`, `UG-${new Date().getUTCFullYear()}-${randomBytes(5).toString('hex').toUpperCase()}`, purchaserName, purchaserEmail.toLowerCase(), createHash('sha256').update(token).digest('hex'), total.toString(), idempotencyKey ?? null])
