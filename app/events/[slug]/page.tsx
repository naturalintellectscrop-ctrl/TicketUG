...
export default async function PublicEventPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="public-event"><p className="eyebrow">TicketUG event</p><h1>{event.title}</h1><p className="eyebrow">Public event page</p><p className="lede">{event.description}</p><div className="event-meta"><span>{new Date(event.starts_at).toLocaleString('en-UG', { dateStyle: 'full', timeStyle: 'short', timeZone: event.timezone })}</span><span>{event.timezone}</span>{event.venue_name && <span>{event.venue_name}{event.venue_city ? `, ${event.venue_city}` : ''}</span>}</div>{salesOpen && anyAvailable && <div className="row"><Link className="button button-primary" href={`/events/${slug}/order`}>Get tickets</Link></div>}{event.lifecycle_state === 'CANCELLED' && <div className="surface"><p role="alert"><strong>This event has been cancelled.</strong> Contact the organizer about any tickets you already hold.</p></div>}<section className="stack"><div className="section-heading"><div><p className="eyebrow">Ticket types</p><h2>Choose your experience</h2></div></div>{tickets.length ? tickets.map((ticket) => <article className="surface" key={ticket.public_id}><div className="row-between"><div><h3>{ticket.name}</h3><p className="lede">{ticket.description}</p></div><strong>{Number(ticket.price_minor_units).toLocaleString('en-UG')} {ticket.currency}</strong></div><p>{availability(ticket)}</p></article>) : <div className="surface"><p>Ticket types will be announced soon.</p></div>}</section></main>
}
