...
export default async function OrganizerTicketsPage({ params }: { params: Promise<{ organizerId: string; eventId: string }> }) {
  ...
  return <main className="page-shell"><Link href={`/organizer/${organizerId}/events/${eventId}`}>Back to event</Link><div className="section-heading"><div><p className="eyebrow">Issued tickets</p><h1>{event.title}</h1></div><span>{tickets.length} issued</span></div><section className="stack">{tickets.length ? tickets.map((ticket) => <article className="surface" key={ticket.public_id}><div className="row-between"><div><h2>{ticket.ticket_type_name_snapshot}</h2><p>{ticket.attendee_name} · {ticket.attendee_email}</p><p>Order {ticket.order_number} · issued {new Date(ticket.issued_at).toLocaleString('en-UG')}</p></div><div><strong>{ticket.status}</strong><p>{ticket.public_id}</p></div></div></article>) : <div className="surface"><p>No tickets have been issued for this event.</p></div>}</section></main>
}
