var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
import { Body, Controller, Get, Param, Post } from '@nestjs/common';
import { ApiTags } from '@nestjs/swagger';
import { CurrentUser } from '../auth/current-user.decorator';
import { CheckInsService } from './check-ins.service';
let CheckInsController = class CheckInsController {
    checkIns;
    constructor(checkIns) {
        this.checkIns = checkIns;
    }
    events(user) { return this.checkIns.listAssignedEvents(user); }
    scan(user, eventId, body) { return this.checkIns.scan(user, eventId, body?.payload ?? ''); }
    summary(user, eventId) { return this.checkIns.summary(user, eventId); }
};
__decorate([
    Get('events'),
    __param(0, CurrentUser()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", void 0)
], CheckInsController.prototype, "events", null);
__decorate([
    Post('events/:eventId/scan'),
    __param(0, CurrentUser()),
    __param(1, Param('eventId')),
    __param(2, Body()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, String, Object]),
    __metadata("design:returntype", void 0)
], CheckInsController.prototype, "scan", null);
__decorate([
    Get('events/:eventId/summary'),
    __param(0, CurrentUser()),
    __param(1, Param('eventId')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, String]),
    __metadata("design:returntype", void 0)
], CheckInsController.prototype, "summary", null);
CheckInsController = __decorate([
    ApiTags('check-ins'),
    Controller('check-ins'),
    __metadata("design:paramtypes", [CheckInsService])
], CheckInsController);
export { CheckInsController };
