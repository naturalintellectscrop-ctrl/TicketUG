...
import { assertTransition, EVENT_STATES } from './event-lifecycle';
...
let EventsController = class EventsController {
    ...
    async ownedEvent(user, eventId, roles = writeRoles) {
        const result = await this.db.query('SELECT id, organizer_id, lifecycle_state FROM ticketug.event WHERE id = $1', [eventId]);
    ...
    async update(user, eventId, body) { const event = await this.ownedEvent(user, eventId); const result = await this.db.query('UPDATE ticketug.event SET title = COALESCE($2,title), description = COALESCE($3,description), starts_at = COALESCE($4,starts_at), ends_at = COALESCE($5,ends_at), slug = COALESCE($6,slug), venue_id = $7, updated_at = now() WHERE id = $1 RETURNING *', [event.id, body.title ?? null, body.description ?? null, body.startsAt ?? null, body.endsAt ?? null, body.slug ?? null, body.venueId === undefined ? (await this.db.query('SELECT venue_id FROM ticketug.event WHERE id=$1', [event.id])).rows[0].venue_id : body.venueId]); return result.rows[0]; }
    ...
    } const published = body.to === 'PUBLISHED'; return (await this.db.query('UPDATE ticketug.event SET lifecycle_state=$2, publication_state=$3, discoverable=$4, updated_at=now() WHERE id=$1 RETURNING *', [event.id, body.to, published ? 'PUBLIC' : body.to === 'DRAFT' ? 'PRIVATE' : undefined, published])).rows[0]; }
    ...
    async deleteMedia(user, eventId, mediaId) { const event = await this.ownedEvent(user, eventId); await this.db.query('DELETE FROM ticketug.event_media WHERE id=$1 AND event_id=$2', [mediaId, event.id]); return { deleted: true }; }
