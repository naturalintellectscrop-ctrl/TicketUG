...
export class OrdersService {
  ...
  async create(user: ApiUser | null, body: CreateOrderDto, guest = false) {
    ...
    const result = await this.db.transaction(async (client) => {
      ...
      const order = await client.query<OrderRow>(`INSERT INTO ticketug.order (id, public_id, order_number, user_profile_id, purchaser_name, purchaser_email, guest_access_token_hash, currency, total_minor_units, idempotency_key) VALUES ($1,$2,$3,$4,$5,$6,$7,'UGX',$8,$9) RETURNING ${orderSummary}`, [orderId, `ord_${randomBytes(12).toString('hex')}`, this.orderNumber(), user?.profileId ?? null, body.purchaserName.trim(), purchaserEmail, guestHash, total, body.idempotencyKey ?? null])
