...
export class EventsController {
  ...
  private async ownedEvent(user: ApiUser, eventId: string, roles = writeRoles) {
    const result = await this.db.query<{ id: string; organizer_id: string; lifecycle_state: EventLifecycleState; venue_id: string | null }>('SELECT id, organizer_id, lifecycle_state, venue_id FROM ticketug.event WHERE id = $1', [eventId])
    const event = result.rows[0]
    if (!event) throw new NotFoundException('Event not found')
    this.membership(user, event.organizer_id, roles)
    return event
  }
  private async assertVenueOwnership(venueId: string | null | undefined, organizerId: string) {
    if (!venueId) return
    const result = await this.db.query<{ id: string }>('SELECT id FROM ticketug.venue WHERE id = $1 AND organizer_id = $2', [venueId, organizerId])
    if (!result.rows[0]) throw new BadRequestException('Venue does not belong to organizer')
  }
  private assertValidEventWindow(startsAt: string | Date, endsAt: string | Date) {
    if (new Date(endsAt) <= new Date(startsAt)) throw new BadRequestException('Event end must be after start')
  }
  ...
  @Delete('events/:eventId/media/:mediaId') async deleteMedia(@CurrentUser() user: ApiUser, @Param('eventId') eventId: string, @Param('mediaId') mediaId: string) { const event = await this.ownedEvent(user, eventId); const result = await this.db.query('DELETE FROM ticketug.event_media WHERE id=$1 AND event_id=$2', [mediaId,event.id]); if (result.rowCount === 0) throw new NotFoundException('Media not found'); return { deleted: true } }
