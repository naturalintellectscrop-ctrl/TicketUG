...
export class PaymentsController {
  ...
  @Post('public/payments/webhooks/:provider') webhook(@Param('provider') provider: string, @Headers() headers: Record<string, string | undefined>, @Body() body: unknown, @Req() request: { rawBody?: Buffer }) { return this.payments.webhook(provider, headers, body, request.rawBody?.toString('utf8') ?? JSON.stringify(body)) }
}
